...
from util_torch.tune_tensorboard import RunManager
from util_torch.tune_tensorboard import RunBuilder
...
def inference(
    model, dataloader, device, node_cnt, window_size, horizon, forecast_loss, manager=None
):
    forecast_set = []
    target_set = []
    model.eval()
    with torch.no_grad():
        for i, (inputs, target) in enumerate(dataloader):
            ...
            while step < horizon:
                ...
                inputs[:, : window_size - len_model_output, :] = inputs[
                    :, len_model_output:window_size, :
                ].clone()
                inputs[:, window_size - len_model_output :, :] = forecast_result.clone()
                forecast_steps[
                    :, step : min(horizon - step, len_model_output) + step, :
                ] = (
                    forecast_result[:, : min(horizon - step, len_model_output), :]
                    .detach()
                    .cpu()
                    .numpy()
                )
...
def validate(
    ...
    forecast_norm, target_norm = inference(
        model, dataloader, device, node_cnt, window_size, horizon, forecast_loss, manager
    )
    ...
    if is_print:
        print(
            f"NORM: MAPE {score_norm[0]:7.9%}; MAE {score_norm[1]:7.9f}; RMSE {score_norm[2]:7.9f}."
        )
...
def train(train_data, valid_data, args, result_file):
    ...
    if args.optimizer == "RMSProp":
        ...
        my_optim = torch.optim.Adam(
            params=model.parameters(), lr=args.lr, betas=(0.9, 0.999)
        )
    my_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
        optimizer=my_optim, gamma=args.decay_rate
    )
    ...
    for epoch in range(args.epoch):
        ...
        print(
            "| end of epoch {:3d} | time: {:5.2f}s | train_total_loss {:5.4f}".format(
                epoch, (time.time() - epoch_start_time), loss_total / cnt
            )
        )
...
def test(test_data, args, result_train_file, result_test_file):
    ...
    test_loader = torch_data.DataLoader(
        test_set, batch_size=args.batch_size, drop_last=False, shuffle=False, num_workers=0
    )
    performance_metrics = validate(
        model,
        test_loader,
    ...
    print(
        "Performance on test set: MAPE: {:5.2f} | MAE: {:5.2f} | RMSE: {:5.4f}".format(
            mape, mae, rmse
        )
    )
...
def tuning(train_data, valid_data, test_data, args, params):
    manager = RunManager(log_dir="StemGNN", is_clf=False)
    performance = {
        "horizon": [],
        "mae": [],
        "rmse": [],
    ...
    for run in RunBuilder.get_runs(params):
        ...
        args.lr = run.lr
        args.decay_rate = run.decay_rate
        args.leakyrelu_rate = run.leakyrelu_rate
        args.horizon = run.horizon
        ...
        for epoch in range(args.epoch):
            ...
            for inputs, target in train_loader:
                ...
                loss.backward()
                my_optim.step()
                manager.track_loss(loss, inputs, is_train=True)
            ...
            del performance_metrics["mae_node"]
            del performance_metrics["rmse_node"]
            del performance_metrics["mape_node"]

            manager.add_performance(performance_metrics)
            manager.end_epoch()
        ...
        performance_test = test(test_data, args, result_train_file, result_test_file)
        after_evaluation = datetime.now().timestamp()
        print(f"Evaluation took {(after_evaluation - before_evaluation) / 60} minutes")

        performance["horizon"].append(run.horizon)
        performance["mae"].append(performance_test["mae"])
        performance["rmse"].append(performance_test["rmse"])
        performance["mape"].append(performance_test["mape"])
        performance["lr"].append(run.lr)
        performance["decay_rate"].append(run.decay_rate)
        performance["leakyrelu_rate"].append(run.leakyrelu_rate)
