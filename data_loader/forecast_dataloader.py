...
def normalized(data, normalize_method, norm_statistic=None):
    if normalize_method == "min_max":
        ...
        scale = np.asarray(norm_statistic["max"]) - np.asarray(norm_statistic["min"]) + 1e-5
...
def de_normalized(data, normalize_method, norm_statistic):
    if normalize_method == "min_max":
        if not norm_statistic:
            norm_statistic = dict(max=np.max(data, axis=0), min=np.min(data, axis=0))
        scale = np.asarray(norm_statistic["max"]) - np.asarray(norm_statistic["min"]) + 1e-8
        ...
        mean = norm_statistic["mean"]
        std = norm_statistic["std"]
        std = [1 if i == 0 else i for i in std]  # maybe.. only 1 value
        data = data * std + mean
    return data
...
class ForecastDataset(torch_data.Dataset):
    def __init__(
        ...
        df = pd.DataFrame(df)
        df = (
            df.fillna(method="ffill", limit=len(df))  # forward fill, limit: # of fill
            .fillna(method="bfill", limit=len(df))  # backward fill
            .values
        )
        self.data = df
